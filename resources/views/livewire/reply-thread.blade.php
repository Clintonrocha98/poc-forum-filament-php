...
<div class="ml-{{ $indent }} mb-4 p-4 rounded-xl border dark:border-gray-700 bg-gray-100 dark:bg-gray-800 mt-3">
    <div class="flex items-start justify-between gap-3 w-full">
        <div class="flex items-start gap-2 w-full ">
            <img src="https://placehold.co/40x40?text={{ urlencode($reply->user->name) }}" class="w-8 h-8 rounded-xl"/>
            <div class="w-full">
            ...
            {{ $reply->created_at->diffForHumans() }}
                ...
                <div x-data="{ open: false }" class="mt-2 w-full ">
                    <button @click="open = !open"
                            class="flex items-center gap-2 text-sm dark:hover:bg-gray-500 px-2 py-1 rounded-xl">
